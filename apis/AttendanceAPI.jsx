...
export const AttendanceAPI = {
  /** Get Attendance records from the database that meet the specified query parameters */
  getAll: async (conditionObj = false, page = 0, size = 10, search = false, authInfo, cancel = false) => {
    ...
    const { data: response } = await api.request({
      url: `/get-attendances?page=${page}&size=${size}${queryParam}${searchParam}`,
  ...
  createAttendance: async (attendance, cancel = false) => {
    return await api.request({
      ...
      method: "POST",
      data: attendance,
      signal: cancel && cancelApiObject.createAttendance ? cancelApiObject.createAttendance.handleRequestCancellation().signal : undefined,
    });
  },

  /** Update Attendance in the database */
  updateAttendance: async (fields, cancel = false) => {
    return await api.request({
      url: `/update-attendance`,
      headers: { "x-access-token": getLocalStorage("auth").token },
      method: "PATCH",
      data: fields,
      signal: cancel && cancelApiObject.updateAttendance ? cancelApiObject.updateAttendance.handleRequestCancellation().signal : undefined,
    });
  },
};
