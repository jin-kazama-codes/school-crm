...
// GET /api/v1/state/get-all-states
export const GET = withAuth(async (req: NextRequest) => {
  ...
  try {
    ...
    const rows = await prisma.state.findMany({ where: whereClause, orderBy: { name: "asc" } });
...
export const POST = withAuth(async (req: NextRequest, { userId }) => {
  ...
  try {
    ...
    const state = await prisma.state.create({ data: payload });
    return NextResponse.json(Utility.formatResponse(200, { id: state.id }), { status: 200 });
  } catch (err) {
...
export const PATCH = withAuth(async (req: NextRequest, { userId }) => {
  ...
  try {
    ...
    await prisma.state.update({ where: { id }, data });
